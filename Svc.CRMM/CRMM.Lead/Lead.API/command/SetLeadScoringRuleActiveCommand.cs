...
public sealed class SetLeadScoringRuleActiveCommand
{
    private readonly ILeadDbContext _db;

    public SetLeadScoringRuleActiveCommand(ILeadDbContext db)
    {
        _db = db;
    }

    public async Task<LeadScoringRuleDto?> ExecuteAsync(Guid id, bool isActive, CancellationToken cancellationToken)
    {
